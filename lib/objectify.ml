...
let rec trans_eq (m, si, j, s) (lhs, rhs) =
  ...
  | PTuple ps, _ ->
      let var = new_var () in
      let m', si', j', s' =
        trans_eq (m, si, j, s) (Norm_builder.pvar var lhs.pat_ty, rhs)
      in
      (m', si', j', s' @ [ tuple_destr ps var ])
  | PVar v, EBase e -> (m, si, j, s @ [ assign v (trans_expr m e) ])
  | _ -> assert false
